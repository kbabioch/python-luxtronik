...
class TestIPAddress:
    ...
    def test_from_heatpump(self):
        ...
        assert IPAddress("").from_heatpump(4294967040) == '255.255.255.0'

        assert IPAddress("").from_heatpump(-1) == '255.255.255.255'
        assert IPAddress("").from_heatpump(0) == '0.0.0.0'
        assert IPAddress("").from_heatpump(1) == '0.0.0.1'
        assert IPAddress("").from_heatpump(2**32+1) == '0.0.0.1'

    def test_to_heatpump(self):
        """Test cases for to_heatpump function"""

        assert IPAddress("").to_heatpump('1.2.3.4') == 16909060
        assert IPAddress("").to_heatpump('255.255.255.0') == 4294967040

        assert IPAddress("").to_heatpump(14294967040) == 4294967040
